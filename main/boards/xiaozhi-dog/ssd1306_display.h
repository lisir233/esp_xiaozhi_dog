...
public:
        ...
        if (square1 && square2) {
            if (show) {
                ...
                if (blink_task_handle == NULL) {
                    ESP_LOGI(TAG, "create blink_task");
                    xTaskCreate([](void* arg) {
                        auto this_ = (Ssd1306Display*)arg;
                        this_->blink_task();
                        vTaskDelete(NULL);
                    }, "blink_task", 3096, this, 5, &blink_task_handle);
                }
                if (idle_task_handle == NULL) {
                    ESP_LOGI(TAG, "create idle_task");
                    xTaskCreate([](void* arg) {
                        auto this_ = (Ssd1306Display*)arg;
    ...
    void UpdateBatteryStatus(bool is_charging, uint8_t battery_level);
};
